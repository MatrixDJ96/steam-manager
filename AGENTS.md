...
./scripts/build.sh                          # PyInstaller --onefile → dist/steam-manager (~20 MB) + dist/steam-manager.sha256
```
...
`cli/config_cmd.py` uses `tomlkit` (not stdlib `tomllib`) because writes must preserve user comments. The PyInstaller build hidden-imports it; if you add modules with similar non-trivial import requirements, declare them in `scripts/build.sh`. The Textual TUI needs `--collect-submodules textual` (its widgets are lazy-loaded via a package `__getattr__` that PyInstaller's static analysis misses — without it the frozen binary raises an import error when the TUI opens) plus `--hidden-import platformdirs`; `app.tcss` rides the existing `--collect-data steam_manager`. A real-terminal smoke of the frozen binary is the only way to catch a packaging-only TUI failure.
...
- `AGENTS.md` — this file: the canonical agent playbook for repo quirks (Claude reads it via the `CLAUDE.md` bridge).
