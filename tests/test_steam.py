...
def test_discover_finds_steam_root_and_libraries(fake_steam: Path):
    ctx = steam.discover(steam_root=fake_steam)
...
def test_library_label_helper(fake_steam: Path):
    ctx = steam.discover(steam_root=fake_steam)
    assert steam.library_label(ctx, fake_steam) == "TestLinux"
...
def test_get_compat_tool_existing_appid(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    assert steam.get_compat_tool(ctx, "111") == "Proton-CachyOS Latest"


def test_get_compat_tool_missing_appid(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    assert steam.get_compat_tool(ctx, "999") is None


def test_set_compat_tool_updates_existing(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    steam.set_compat_tool(ctx, "111", "GE-Proton 9-20")
    assert steam.get_compat_tool(ctx, "111") == "GE-Proton 9-20"


def test_set_compat_tool_creates_new_mapping(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    steam.set_compat_tool(ctx, "222", "Proton-CachyOS Latest")
    assert steam.get_compat_tool(ctx, "222") == "Proton-CachyOS Latest"


def test_get_launch_options_existing(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    user = next(u for u in steam.list_users(ctx) if u.is_active)
    assert steam.get_launch_options(user, "111") == "scopebuddy -- %command%"


def test_get_launch_options_missing(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    user = next(u for u in steam.list_users(ctx) if u.is_active)
    assert steam.get_launch_options(user, "999") is None


def test_set_launch_options_updates(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    user = next(u for u in steam.list_users(ctx) if u.is_active)
    steam.set_launch_options(user, "111", "DXVK_FRAME_RATE=0 scopebuddy -- %command%")
    assert steam.get_launch_options(user, "111") == "DXVK_FRAME_RATE=0 scopebuddy -- %command%"


def test_set_launch_options_creates_apps_section_and_entry(fake_steam):
    ctx = steam.discover(steam_root=fake_steam)
    user = next(u for u in steam.list_users(ctx) if u.is_active)
    steam.set_launch_options(user, "222", "scopebuddy -- %command%")
    assert steam.get_launch_options(user, "222") == "scopebuddy -- %command%"
...
''')
    ...
    assert steam.get_launch_options(user, "555") == "scopebuddy -- %command%"
